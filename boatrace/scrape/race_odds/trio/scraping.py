...
from boatrace.scrape.race_odds.trio import OddsTrioConst
...
class OddsTrioScraping:
    ...
    def scrape(self, race_id: int, stadium_id: int, date: datetime) -> list[dict]:
        ...
        td_soups = soup.find("tbody", class_=OddsTrioConst.tbody_class).find_all("td")
    ...
    def _extract_odds(self, race_id: int, stadium_id: int, date: datetime, line_td_soup: Tag, idx_line: int) -> list[dict]:
        n_block = idx_line + 1
        fs14_td_soups = [td_soup for td_soup in line_td_soup if OddsTrioConst.td_class in td_soup.attrs["class"]]
        td_soups = [td_soup for td_soup in line_td_soup if (OddsTrioConst.td_class not in td_soup.attrs["class"] and
                                                            OddsTrioConst.disable_class not in td_soup.attrs["class"])]
        ...
        for idx_1st, (soup_2nd, soup_3rds) in enumerate(zip(fs14_td_soups, block_td_soups)):
            for no_soup, data_soup in soup_3rds:
                ...
                scrape_datas.append(OddsTrioColumns().cast([
                    race_id,
                    stadium_id,
                    date,
                    bet_no,
